...
\item
  ...
  monitoring data} The variables included in this dataset are:
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{unzip}\NormalTok{(}\StringTok{"activity.zip"}\NormalTok{,}\DataTypeTok{exdir =} \StringTok{"Data"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Just recreating activityDT from scratch then making the new factor
variable. (No need to, just want to be clear on what the entire process
is.)
